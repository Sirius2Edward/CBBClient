...
-(BOOL)isEmailAddress
{
    ...
    if (!result) {
        [SVProgressHUD showErrorWithStatus:@"这不是一个正常的Email！"];
    }    
    return result;
}
...
-(BOOL)isMobileNumber
{
    ...
    if (!result) {
        [SVProgressHUD showErrorWithStatus:@"这不是一个正常的手机号码！"];
    }
    return result;
}
...
{
    if ([self isEqualToString:@""]) {
        [SVProgressHUD showErrorWithStatus:@"区号不能为空！"];
        return NO;
    }
    ...
    if (!result) {
        [SVProgressHUD showErrorWithStatus:@"这不是一个正常的区号！"];
    }
    return result;
}
...
-(BOOL)isTelNumber
{
    ...
    if (!result) {
        [SVProgressHUD showErrorWithStatus:@"这不是一个正常的电话号码！"];
    }
    return result;
}
...
-(BOOL)isBranchNumber
{
    ...
    if (!result) {
        [SVProgressHUD showErrorWithStatus:@"分机号超长！"];
    }
    return result;
}
...
-(BOOL)isPhoneNumber
{
    ...
    if (arr.count<2 || arr.count>3) {
        [SVProgressHUD showErrorWithStatus:@"联系电话输入有误！"];
        return NO;
    }
    else {
...
-(BOOL)isQQNumber
{
    ...
    BOOL result = [branchTest evaluateWithObject:self];
    if (!result) {
        [SVProgressHUD showErrorWithStatus:@"QQ号不合法！"];
    }
    return result;
}
...
{
    if ([self isEqualToString:@""]) {
        [SVProgressHUD showErrorWithStatus:[NSString stringWithFormat:@"%@不能为空！",name]];
        return nil;
    }
    ...
    {
        [SVProgressHUD showErrorWithStatus:[NSString stringWithFormat:@"该%@不在列表中！",name]];
        return nil;
    }    
}
...
{
    if ([self isEqualToString:@""]) {
        [SVProgressHUD showErrorWithStatus:@"省市不能为空！"];
        return nil;
    }
    NSArray *arr = [self componentsSeparatedByString:@" - "];
    if (arr.count != 2) {
        [SVProgressHUD showErrorWithStatus:@"请按“省 - 市”格式输入！"];
        return nil;
    }
    
    NSArray *sheng = [cityDic objectForKey:[arr objectAtIndex:0]];
    if (!sheng) {
        [SVProgressHUD showErrorWithStatus:@"没有该省！"];
        return nil;
    }
    ...
    if (!cityID) {
        [SVProgressHUD showErrorWithStatus:@"没有该市！"];
        return nil;
    }
...
-(NSString *)getCityID:(NSDictionary *)cityDic
{
    ...
    if (!cityID) {
        [SVProgressHUD showErrorWithStatus:@"没有该城市！"];
    }
    return cityID;
}
@end
